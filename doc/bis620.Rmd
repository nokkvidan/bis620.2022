...
load(file="~/bis620.2022/data/diabetes.rda")
...
# change all variables to categorical, except for Age and BMI
train <- train %>%
...
detach(package:bis620.2022, unload=TRUE)
devtools::install_github("nokkvidan/bis620.2022@forest", force=TRUE)
library(bis620.2022)
```
...
```{r}
glm_fit <- glm_model("Diabetes_binary", train = train[-c(4, 13)], test = test[-c(4, 13)], optimize = "manual")
summary(glm_fit)
```
