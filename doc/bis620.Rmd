...
# Load the data
# devtools::install_github("nokkvidan/bis620.2022@temp")
data(diabetes)
...
To conduct a counter factual analysis, we first begin with finding a set of variables that were marked as significant by each of the models (according to their own metrics), or at the least, variables in which the majority of models marked them as important. We also constrained our selection to actionable indicators so that a patient could take action for change or reduce the risk of it occurring. For example, the patient could lower the BMI through exercise, but could not change their age. By keeping only actionable indicators, the results of this analysis can help a doctor give the patient a concrete set of improvements to their lives to reduce the likelihood of diabetes.

We proceeded by initially fitting our model on a subset of the test data were patients were diabetic and belonged to the class of our specified variable. For example, diabetic patients who had a “high cholesterol” value of 1. The actionable and important indicators that resulted from these tests were blood pressure, cholesterol, BMI and heart disease/attack. All of these, except for BMI, are categorical variables.

Next, we examine the initial prediction for the proportion of individuals we predicted to have diabetes in our model. We changed the value of the variable (the counter factual), to a healthy value (or range in the case of BMI) and noticed the increase/decrease in the proportion of individuals being classified as having diabetes.

In each of the models and for all of our variables, performing a counter factual change reduced the proportion of individuals being classified as having type 2 diabetes. Changing the BMI had the greatest effect in two of our models and the 2nd biggest effect in the other. Thus, the best action to take would be to reduce a patients BMI. The next best actions are to reduce the blood pressure and cholesterol levels, which the doctor could also provide remedies for. It seems that the heart disease or attack had consistently the smallest decreases amongst all models so should be given least priority amongst them.

The output of the analysis referenced above is presented in the tables below:
